...
def functioncall(command_message):
    ...
    prompt = (
            ...
            "new_content参数部分请你用markdown的格式输出\n"
            ...
            "content_to_insert参数部分请你用markdown的格式输出\n"
            "6.read_file_with_line_numbers 这个指令用于读取文件的内容，并返回带有行号标识的内容信息。  参数为file_path\n"
            
            "你需要遵守的规则：\n"
            "1.你只需要回复提取后的信息就行，不要回复其他多余的话。例如输入为 创建一个工作室，你应该只回复 create_studio\n"
            "2.如果指令包含参数，指令名和参数，以及参数和参数之间用 , 间隔，并且回复参数相关的内容的时候只需要回复实参的名字，不要其他的！！"
            "如某个参数为code.py，不要回复file_path=code.py"
            "需要转化的指令信息为：\n" + command_message
    )
    # 调用模型
    messages = [
    ...
    response = " "
    for response_part in chat_with_ai(messages, "gpt-4-turbo"):
